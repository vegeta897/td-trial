...
export default class PhysicsSystem extends System {
	...
	update(tick: number) {
		this.bodies.each((entity, { body }, transform) => {
			...
			if (transform.lastUpdated === tick) {
				body.position.copy(<CANNON.Vec3>(<unknown>transform.position))
				body.quaternion.copy(<CANNON.Quaternion>(<unknown>transform.rotation))
			}
			if (body.type === CANNON.Body.STATIC) return
			const velocity = this.world.get(entity, Velocity3D)
			if (velocity)
				body.velocity.copy(
					<CANNON.Vec3>(<unknown>velocity.vector3.clone().multiplyScalar(60))
				)
